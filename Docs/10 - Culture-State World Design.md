...
**Why Iberia first:** it exercises the entire ladder in the smallest package. The initial culture-state set can reuse existing vanilla tags after the Phase 1B audit (`CAS`, `ARA`, `CAT`, `GAL`, `LON`, `NAV`, `GRA`, `POR`), with `ARA` requiring primary-culture history review; a clean peninsula boundary; a vanilla formable target (`SPA`) with an owner-decided rule already in place (Spain excludes Portugal); a competing formable (`ADU`) to test rival formation; and the flagship colonizers (Castile, Portugal) to later test the unification-then-colonization sequence end to end. Phase 2A.1 adds the POC-specific southern Portuguese Muslim rival `AGH` (Al-Gharb) because no suitable vanilla Algarve/Gharb tag exists; this is a Portuguese-culture split of the southern block, not a map-layer change and not a North African transplant.
